  ...
  return (
    ...
    >
      <div className="flex justify-end mb-2 gap-2">
        <div className="p-1 bg-gray-600 text-white text-[0.5rem] sm:text-xs rounded-md">
          {move.type.replace("_", " ")}
        </div>
        {(move.type === "POWER_UP" || move.type === "VIRUS") && (
          <div
            ...
            } text-white text-[0.5rem] sm:text-xs rounded-md`}
          >
      ...
      <div className="text-left">
        <div className="font-semibold text-sm sm:text-md">{move.name}</div>
        <div className="text-sm sm:text-base">POWER: {move.power}</div>
      </div>
    </button>
  );
}
